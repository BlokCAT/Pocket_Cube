...
char  Version_Information[] = "0 .2 .0";
...
void EX_mouseCallback(ExMessage msg);
...
void Inite_EasyX()
{
    initgraph(700, 700);//easyX窗口
    setbkcolor(RGB(254, 243, 197));
    cleardevice();


    loadimage(&Show_How_Use, "assets/Show_How_Use.jpg", 700, 820); // 加载图像，最后一个参数是背景色索引值（默认-1表示不透明）
    putimage(0, 0, &Show_How_Use); // 将图像放置在窗口左上角


    setbkmode(TRANSPARENT);

    Draw_TEXT(40, 370, 600, 60, "按 下 回 车 复 原（目前版本只可以复原5步以内的打乱）");
...
void AUTOMATIC_RECOVERY(int idx)
{
    ...
    else//复原完成时
    {
        ...
        Sleep(200);
        Draw_TEXT(40, 450, 600, 60, "复 原 完 成！");
